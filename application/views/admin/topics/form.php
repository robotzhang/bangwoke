<form class="form-horizontal" method="post" action="<?php echo site_url(empty($topic) ? 'admin/topics/create' : 'admin/topics/update') ?>">
    <fieldset>
        <legend><?php echo empty($topic) ? '新建' : '编辑' ?>电影专题</legend>
        ...
        <div class="control-group">
            ...
            <div class="controls">
                <input type="text" name="topic[discount_price]" class="input-xlarge" value="<?php if (!empty($topic)) echo $topic->discount_price ?>">
            </div>
        </div>
        <div class="control-group">
